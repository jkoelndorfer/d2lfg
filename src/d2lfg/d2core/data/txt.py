...
class Diablo2TxtRecord(Sequence[str], Mapping[str, str]):
    ...
    def __getitem__(self, k: Union[slice, str, int]) -> Union[str, Sequence[str]]:
        """
        Given a key, fetches a value from the record.

        If the key is an integer, the field at the given 0-indexed position
        is returned. If the key is a string, the field appearing in that
        column is returned.
    ...
    def __repr__(self) -> str:
        """
        Returns a representation of the record as a string. This representation
        is not passable to :py:func:`eval`.
        """
...
class Diablo2TxtParser:
    ...
    def parse(self, file: Union[Path, str, TextIOBase]) -> Diablo2TxtFile:
        """
        Parses the given file into a :py:class:`Diablo2TxtFile`.

        :param file: a file path or text IO object to parse
        """
